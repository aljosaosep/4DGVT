...
namespace SUN {
    namespace utils {
        namespace scene_flow {
            ...
            void FreeMatcher(libviso2::Matcher *&matcher) {
                if (matcher) {
                    std::cout << "Info: matcher memory freed." << std::endl;
                    delete matcher;
                    matcher = nullptr;
                }
            }
            ...
            GetSceneFlow(std::vector<libviso2::Matcher::p_match> quad_matches, const Eigen::Matrix4d Tr,
                         const SUN::utils::Camera &camera,
                         float baseline, double dt, float max_velocity_ms) {
                ...
                for (const auto &match:quad_matches) {
                    ...
                    Eigen::Vector4d p3d_c = ProjTo3D(match.u1c, match.u2c, match.v1c, camera,
                                                     baseline); // p3d curr frame
                    Eigen::Vector4d p3d_p = ProjTo3D(match.u1p, match.u2p, match.v1p, camera,
                                                     baseline); // p3d prev frame
                    ...
                    int max_dist = 90;
                    int max_lat = 30;
                    if (std::fabs(p3d_c[0]) > max_lat || std::fabs(p3d_c[2]) > max_dist ||
                        std::fabs(p3d_p[0]) > max_lat || std::fabs(p3d_p[2]) > max_dist)
                        continue;
            ...
            void InitVO(std::shared_ptr<libviso2::VisualOdometryStereo> &vo, double f, double c_u, double c_v,
                        double baseline) {
                if (vo == nullptr) {
            ...
            Eigen::Matrix4d EstimateEgomotion(libviso2::VisualOdometryStereo &viso, const cv::Mat &color_left,
                                              const cv::Mat &color_right) {
                cv::Mat grayscale_left, grayscale_right;
                cv::cvtColor(color_left, grayscale_left, cv::COLOR_BGR2GRAY);
                cv::cvtColor(color_right, grayscale_right, cv::COLOR_BGR2GRAY);
                const int32_t width = color_left.cols;
