...
namespace SUN {
    namespace utils {
        namespace KITTI {
            ...
            Eigen::Matrix<double, 4, 4> Calibration::GetTr_cam0_cam3() const {
                Eigen::Matrix<double, 4, 4> m = Eigen::Matrix<double, 4, 4>::Identity();
                m.block<3, 1>(0, 3) = pos_cam_3_.block<3, 1>(0, 0);
                return m;
            }
            ...
            int LabelsIO::ReadLabels(const std::string &filename, std::vector<TrackingLabel> &labels) {
                ...
                while (getline(file, line)) {
                    ...
                    if (type.compare("CAR") == 0) {
                        ...
                        label.type = SUN::shared_types::STATIC_BACKGROUND;
                    } else {
                        label.type = SUN::shared_types::MISC;
                    }
            ...
            int LabelsIO::ReadLabels3DOP(const std::string &filename, std::vector<TrackingLabel> &labels) {
                ...
                std::string line;
                while (getline(file, line)) {
                    std::vector<std::string> strings(17); // Each line has 30 space-separated entries
                    boost::split(strings, line, boost::is_any_of(" "));
                    TrackingLabel label;
            ...
            int LabelsIO::WriteLabels(const std::vector<TrackingLabel> &labels, const std::string &filename) {
            ...
            void LabelsIO::ConvertLineToMatrix(std::string &line, Eigen::Matrix<double, 3, 4> &matrix) {
                ...
                for (size_t i = 0; i < rows; i++) {
                    for (size_t j = 0; j < cols; j++) {
                        size_t index =
                                (cols * i) + j + offset; // + 1 is to ignore the first column which indicates the name
