...
namespace SUN {
    namespace utils {
        ...
        bool Camera::ComputeMeasurementCovariance3d(const Eigen::Vector3d &pos_3d, Eigen::Matrix2d c2d_mat,
                                                    const Eigen::Matrix<double, 3, 4> &P_left_cam,
                                                    const Eigen::Matrix<double, 3, 4> &P_right_cam,
                                                    Eigen::Matrix3d &covariance_out) {
            ...
            double wl = P_left_cam(2, 0) * pos_3d[0] + P_left_cam(2, 1) * pos_3d[1] + P_left_cam(2, 2) * pos_3d[2] +
                        P_left_cam(2, 3);
            double wl2 = wl * wl;

            double wr = P_right_cam(2, 0) * pos_3d[0] + P_right_cam(2, 1) * pos_3d[1] + P_right_cam(2, 2) * pos_3d[2] +
                        P_right_cam(2, 3);
            double wr2 = wr * wr;
            ...
            for (int i = 0; i < 2; i++) {
                FL(i, 0) = P_left_cam(i, 0) / wl -
                           (P_left_cam(i, 0) * pos_3d[0] + P_left_cam(i, 1) * pos_3d[1] + P_left_cam(i, 2) * pos_3d[2] +
                            P_left_cam(i, 3)) * P_left_cam(2, 0) / wl2;
                FL(i, 1) = P_left_cam(i, 1) / wl -
                           (P_left_cam(i, 0) * pos_3d[0] + P_left_cam(i, 1) * pos_3d[1] + P_left_cam(i, 2) * pos_3d[2] +
                            P_left_cam(i, 3)) * P_left_cam(2, 1) / wl2;
                FL(i, 2) = P_left_cam(i, 2) / wl -
                           (P_left_cam(i, 0) * pos_3d[0] + P_left_cam(i, 1) * pos_3d[1] + P_left_cam(i, 2) * pos_3d[2] +
                            P_left_cam(i, 3)) * P_left_cam(2, 2) / wl2;

                FR(i, 0) = P_right_cam(i, 0) / wr - (P_right_cam(i, 0) * pos_3d[0] + P_right_cam(i, 1) * pos_3d[1] +
                                                     P_right_cam(i, 2) * pos_3d[2] + P_right_cam(i, 3)) *
                                                    P_right_cam(2, 0) / wr2;
                FR(i, 1) = P_right_cam(i, 1) / wr - (P_right_cam(i, 0) * pos_3d[0] + P_right_cam(i, 1) * pos_3d[1] +
                                                     P_right_cam(i, 2) * pos_3d[2] + P_right_cam(i, 3)) *
                                                    P_right_cam(2, 1) / wr2;
                FR(i, 2) = P_right_cam(i, 2) / wr - (P_right_cam(i, 0) * pos_3d[0] + P_right_cam(i, 1) * pos_3d[1] +
                                                     P_right_cam(i, 2) * pos_3d[2] + P_right_cam(i, 3)) *
                                                    P_right_cam(2, 2) / wr2;
            }
        ...
        bool Camera::ComputeMeasurementCovariance3d(const Eigen::Vector3d &pos_3d, double c_2d,
                                                    const Eigen::Matrix<double, 3, 4> &P_left_cam,
                                                    const Eigen::Matrix<double, 3, 4> &P_right_cam,
                                                    Eigen::Matrix3d &covariance_out) {
        ...
        void Camera::init(const Eigen::Matrix<double, 3, 4> &reference_projection_matrix,
                          const Eigen::Matrix4d &odometry_matrix, int width, int height) {
            ...
            reference_pose_(0, 0) = (ref_proj_mat_copy(0, 3) - ref_proj_mat_copy(0, 2) * reference_pose_(2, 0)) /
                                    ref_proj_mat_copy(0, 0);
            reference_pose_(1, 0) = (ref_proj_mat_copy(1, 3) - ref_proj_mat_copy(1, 2) * reference_pose_(2, 0)) /
                                    ref_proj_mat_copy(1, 1);
        ...
        double Camera::ComputeGroundPlaneHeightInWorldSpace(double pose_plane_x, double pose_plane_z,
                                                            const Eigen::Vector4d &plane_camera_space) const {
        ...
        Eigen::Vector3d Camera::ProjectPointToGroundInWorldSpace(const Eigen::Vector3d &p_world,
                                                                 const Eigen::Vector4d &plane_camera_space) const {
            Eigen::Vector4d plane_world = this->PlaneCameraToWorld(plane_camera_space);
            const double plane_d_world = plane_world[3];
            double a = plane_world[0];
            ...
            double dist = DistancePointToGroundInWorldSpace(p_world,
                                                            plane_camera_space);
            p_proj -= dist *
                      Eigen::Vector3d(a, b, c); // Substract plane_normal*dist_to_plane, and we have a projected point!

            return p_proj;
        }

        double Camera::DistancePointToGroundInWorldSpace(const Eigen::Vector3d &p_world,
                                                         const Eigen::Vector4d &plane_camera_space) const {
            Eigen::Vector4d plane_world = this->PlaneCameraToWorld(plane_camera_space);
            const double plane_d_world = plane_world[3];
            double a = plane_world[0];
