...
namespace SUN {
    namespace utils {
        ...
        public:
            virtual double
            DistanceToGround(const Eigen::Vector3d &p) const = 0; // Compute distance of 3d point to ground
            virtual Eigen::Vector3d
            ProjectPointToGround(const Eigen::Vector3d &p) const = 0; // Compute projection of 3d point to ground
            virtual Eigen::Vector3d
            IntersectRayToGround(const Eigen::Vector3d &ray_origin, const Eigen::Vector3d &ray_direction) const = 0;

            virtual Eigen::Vector3d
            Normal(const Eigen::Vector3d &point_3d) const = 0; // Returns normal at the specified 3d (ground) point
            virtual void
            FitModel(pcl::PointCloud<pcl::PointXYZRGBA>::ConstPtr point_cloud_in, double height_threshold) = 0;
        };
        ...
        public:
            ...
            /**
              ...
              */
            Eigen::Vector3d
            IntersectRayToGround(const Eigen::Vector3d &ray_origin, const Eigen::Vector3d &ray_direction) const;

            /**
              * @brief Gives you a normal of the ground representation at a given query point.
              */
            Eigen::Vector3d
            Normal(const Eigen::Vector3d &point_3d) const; // Returns normal at the specified 3d (ground) point
