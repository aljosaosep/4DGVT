...
namespace SUN {
    namespace utils {
        ...
        public:
            ...
            /**
               * @brief This method initializes camera: K, [R|t] and their inverse matrices,
               * reference pose, width, height.
               * With this initialization method, reference_projection_matrix is assumed to be proj. matrix
               * in the reference frame, which is K*I 3x4 matrix. odometry_matrix represents
               * camera-frame transformation from reference frame to current frame.
               * This matrix can be obtained e.g. using visual odometry method
               * (or any other approach that provides robot's odometry).
               */
            void
            init(const Eigen::Matrix<double, 3, 4> &reference_projection_matrix, const Eigen::Matrix4d &odometry_matrix,
           ...
           */
            ...
            /**
              ...
              */
            double ComputeGroundPlaneHeightInWorldSpace(double pose_plane_x, double pose_plane_z,
                                                        const Eigen::Vector4d &plane_camera_space) const;

            Eigen::Vector3d ProjectPointToGroundInWorldSpace(const Eigen::Vector3d &p_world,
                                                             const Eigen::Vector4d &plane_camera_space) const;

            double DistancePointToGroundInWorldSpace(const Eigen::Vector3d &p_world,
                                                     const Eigen::Vector4d &plane_camera_space) const;
            ...
            /**
               ...
               * @return TODO.
               */
            static bool ComputeMeasurementCovariance3d(const Eigen::Vector3d &pos_3d, Eigen::Matrix2d c2d_mat,
            ...
            /**
               ...
               * @return TODO.
               */
            static bool ComputeMeasurementCovariance3d(const Eigen::Vector3d &pos_3d, double c_2d,
                                                       const Eigen::Matrix<double, 3, 4> &P_left_cam,
                                                       const Eigen::Matrix<double, 3, 4> &P_right_cam,
                                                       Eigen::Matrix3d &covariance_out);
