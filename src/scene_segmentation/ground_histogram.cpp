...
namespace GOT {
    namespace segmentation {
        ...
        void GroundHistogram::ComputeDensityMap(pcl::PointCloud<pcl::PointXYZRGBA>::ConstPtr input_cloud,
                                                const SUN::utils::Camera &camera,
                                                bool distance_compensate, /*bool is_organized_cloud,*/
                                                const double normalization_factor, const double threshold) {
            ...
            for (const auto &p:tmp_cloud) {
                if (!std::isnan(p.x)) {
                    ...
                    if (distance_compensate)
                        this->density_map_(bin_z, bin_x) += std::max(1.0,
                                                                     input_cloud_z * (std::log(input_cloud_z) / log_2));
                    else
        ...
        bool
        GroundHistogram::GetBinFrom3dPoint(const SUN::utils::Camera &camera, const Eigen::Vector3d &point_3d, int &row,
                                           int &col) const {
            row = col = -1;
        ...
        const Eigen::Matrix<std::vector<int>, Eigen::Dynamic, Eigen::Dynamic> &
        GroundHistogram::point_index_map() const {
